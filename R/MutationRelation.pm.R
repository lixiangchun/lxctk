...
#mutation_file = as.character(commandArgs()[2]);
#permutations = as.numeric(commandArgs()[3]);
#output_file = as.character(commandArgs()[4]);
...
{
    if (!is.null(mut.file)) read.table(mut.file)->x
	x[x>0]=1
    ...
    names(tt)= c("Gene1","Gene2","CntGene1","CntGene2","AndCnt","XorCnt","Perm_CntGene1","Perm_CntGene2","Perm_AndCnt","Perm_XorCnt","Coocurrence_Pvalue","Exclusivity_Pvalue")
	tt$Coocurrence_FDR = p.adjust(tt$Coocurrence_Pvalue, "fdr")
	tt$Exclusivity_FDR = p.adjust(tt$Exclusivity_Pvalue, "fdr")
...
#mut_cor_permu_test(mut.file='../Colorectum.gene_mut_mat.txt', n.permu=100, out.file='out.txt')
#cat(mutation_file, permutations, output_file, file=stderr())
#cat("\n", file=stderr())
#mut_cor_permu_test(mut.file=mutation_file, n.permu=permutations, out.file=output_file)
