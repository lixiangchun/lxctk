...
AlexandrovEtAl_signatures_annotation <- function(x, cutoff=0.7, annot_outfile)
{
	#Signature.1A = list(Cancer_types="Signature 1 has been found in all cancer types and in most cancer samples.", Proposed_aetiology="Signature 1 is the result of an endogenous mutational process initiated by spontaneous deamination of 5-methylcytosine.", Additional_mutational_features:)
	Signatures <- list(Signature.1A = "Cancer types: Signature 1 has been found in all cancer types and in most cancer samples.\nProposed aetiology: Signature 1 is the result of an endogenous mutational process initiated by spontaneous deamination of 5-methylcytosine.\nAdditional mutational features: Signature 1 is associated with small numbers of small insertions and deletions in most tissue types.\nComments: The number of Signature 1 mutations correlates with age of cancer diagnosis.",
	Signature.1B = "Cancer types: Signature 1 has been found in all cancer types and in most cancer samples.\nProposed aetiology: Signature 1 is the result of an endogenous mutational process initiated by spontaneous deamination of 5-methylcytosine.\nAdditional mutational features: Signature 1 is associated with small numbers of small insertions and deletions in most tissue types.\nComments: The number of Signature 1 mutations correlates with age of cancer diagnosis.",
...
AlexandrovEtAl_signatures_corr <- function(mutational_processes_file, plot=FALSE, outfile, cutoff=0.7, annot_outfile, ...)
{
	d <- read.table(mutational_processes_file, header=TRUE, stringsAsFactors=FALSE)	
	d <- lxctk::sortDataFrame(d, c('types','subtypes'))
	data('AlexandrovEtAl_signatures')
	Alexandrov <- AlexandrovEtAl_signatures[,4:ncol(AlexandrovEtAl_signatures)]
	lixc <- d[,3:ncol(d)]
	...
	if (plot) {
		txt <- round(x,2)
		txt[txt<0.5] <- NA
		NMF::aheatmap(x, 'Reds', revC = FALSE, Rowv = FALSE, Colv = FALSE, txt=txt)
	}
	# plot with plotly
	#Cosine <- x
	...
	if (!missing(annot_outfile)) {
		AlexandrovEtAl_signatures_annotation(x, cutoff, annot_outfile)
	}
	invisible(x)
}
