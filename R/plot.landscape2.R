...
plot.landscape2 <- function(upper.panel, middle.panel, left.panel, right.panel, legend.panel, subtype.panel=NULL, subtype.2.panel=NULL,
							   ...
							   right.panel.qscore.col='red')
{
	...
	if (group.by.subtype) {
		if (subtype.categs.num > 3) {
			warning("Only 2 subtypes supported at this moment!\n")
		}
		if (subtype.categs.num == 2) {	
			subtype.1.mat <- middle.panel[names(subtype.data[subtype.data==subtype.categs[1]]),]
			subtype.2.mat <- middle.panel[names(subtype.data[subtype.data==subtype.categs[2]]),]
			middle.panel <- rbind(subtype.1.mat, subtype.2.mat)
		} else if (subtype.categs.num == 3) {
			subtype.1.mat <- middle.panel[names(subtype.data[subtype.data==subtype.categs[1]]),]
			subtype.2.mat <- middle.panel[names(subtype.data[subtype.data==subtype.categs[2]]),]
			subtype.3.mat <- middle.panel[names(subtype.data[subtype.data==subtype.categs[3]]),]

			# Deal with subtype that have only 1 sample
			if (sum(subtype.data == subtype.categs[1]) == 1) {
				subtype.1.mat <- t(as.matrix(subtype.1.mat))
				rownames(subtype.1.mat) <- names(subtype.data[subtype.data==subtype.categs[1]])
			}
			if (sum(subtype.data == subtype.categs[2]) == 1) {
				subtype.2.mat <- t(as.matrix(subtype.2.mat))
				rownames(subtype.2.mat) <- names(subtype.data[subtype.data==subtype.categs[2]])
			}
			if (sum(subtype.data == subtype.categs[3]) == 1) {
				subtype.3.mat <- t(as.matrix(subtype.3.mat))
				rownames(subtype.3.mat) <- names(subtype.data[subtype.data==subtype.categs[3]])
			}

			middle.panel <- rbind(subtype.1.mat, subtype.2.mat, subtype.3.mat)
		}
